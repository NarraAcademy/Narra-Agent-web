      ...
      <div className="container">
        ...
        <div className="w-full flex flex-col items-center gap-1">
          {pricing.groups && pricing.groups.length > 0 && (
            <div className="inline-flex h-11 mb-12 items-center rounded-lg bg-muted p-1 text-base">
              <RadioGroup
                value={group}
                className={cn(
                  "h-full grid gap-1",
                  pricing.groups.length === 2 && "grid-cols-2",
                  pricing.groups.length === 3 && "grid-cols-3",
                  pricing.groups.length === 4 && "grid-cols-4"
                )}
                onValueChange={(value) => {
                  setGroup(value);
                }}
                ...
                {pricing.groups.map((item, i) => {
                  return (
                    ...
                    >
                      ...
                      <Label
                        htmlFor={item.name}
                        className="flex h-full cursor-pointer items-center justify-center px-6 py-2 font-medium text-muted-foreground transition-colors peer-data-[state=checked]:text-foreground hover:text-foreground"
                      >
                        {item.title}
                        {item.label && (
                          <Badge
                            variant="outline"
                            className="border-primary bg-primary px-1.5 ml-2 text-xs text-primary-foreground"
                          >
                            {item.label}
                          </Badge>
                        )}
